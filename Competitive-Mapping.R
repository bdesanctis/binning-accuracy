...
tempN = cbind(expand.grid(assignment.list,protocol.list,N.list),
              Ttf=Ttf.base,k=k.base,rT=rT.base,Tqt=Tqt.base)
tempTtf = cbind(expand.grid(assignment.list,protocol.list,Ttf.list),
                N=N.base,k=k.base,rT=rT.base,Tqt=Tqt.base)[,c(1,2,4,3,5,6,7)]
tempk = cbind(expand.grid(assignment.list,protocol.list,k.list),
              N=N.base,Ttf=Ttf.base,rT=rT.base,Tqt=Tqt.base)[,c(1,2,4,5,3,6,7)]
temprT = cbind(expand.grid(assignment.list,protocol.list,rT=rT.list),
               N=N.base,Ttf=Ttf.base,k=k.base,Tqt=Tqt.base)[,c(1,2,4,5,6,3,7)]
tempTqt = cbind(expand.grid(assignment.list,protocol.list,Tqt=Tqt.list),
                N=N.base,Ttf=Ttf.base,k=k.base,rT=rT.base)[,c(1,2,4,5,6,7,3)]

colnames(tempN) = colnames(tempTtf) = colnames(tempk) = 
  colnames(temprT) = colnames(tempTqt) =  c("assignment","protocol","N","Ttf","k","rT","Tqt")
temp = rbind(tempN,tempTtf,tempk,temprT,tempTqt)
...
relN = out.table[1:27,] 
relN$N = factor(relN$N,levels=exact-match(relN$N))
relN$protocol = ifelse(relN$protocol=="exact-match","U","LM")
...
relTtf$Ttf = factor(round(relTtf$Ttf),levels=exact-match(round(relTtf$Ttf)))
...
relk$k = factor(round(relk$k),levels=exact-match(round(relk$k)))
...
relrT$rT = factor(relrT$rT,levels=exact-match(relrT$rT))
...
relTqt$Tqt = factor(relTqt$Tqt,levels=exact-match(relTqt$Tqt))
...
# 3. Creates the paper figure for the example when the two protocols differ. ####
...
temp = expand.grid(assignment.list,protocol.list,N.base,Ttf=Ttf.base,
                   k=k.base,rT=rT.base,Tqt=Tqt.base)
...
dplot = ggplot(reld, aes(x = protocol, y = probability, fill = assignment)) + 
  theme_set(theme_bw(base_size=14) + theme(legend.position="none") ) +
  geom_bar(stat = 'identity', position = 'stack')  +  
